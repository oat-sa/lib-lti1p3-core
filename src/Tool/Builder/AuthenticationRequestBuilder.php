...
{
    public function createQuery(DeploymentInterface $deployment, AuthenticationRequestMessage $authenticationRequestMessage): string
    {
        return sprintf(
            '%s?%s',
            $deployment->getPlatform()->getOidcAuthenticationUrl(),
            http_build_query($authenticationRequestMessage->export())
        );
    }
}
